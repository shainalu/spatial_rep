"""
batch corrected cross dataset
different from 32_batchcrossdataset.py because batch correcting for individual brain areas separately
...
#outfiles
MOD_TEST_OUT = "STtoABA_STtest_f1_0p1areabatch_040821.csv"
MOD_TRAIN_OUT = "STtoABA_STtrain_f1_0p1areabatch_040821.csv"
CROSS_ALL_OUT = "STtoABA_ABAall_f1_0p1areabatch_0408c21.csv"
...
def filterproponto(sampleonto):
    ...
    sampleonto = sampleonto.loc[:,sampleonto_sums > 100] #greater than 5 becuase less is not enough for train/test split to have non-zero areas
...
def findoverlapareas(STonto, propontvox, ontology):
    ...
    print("number of overlapping areas: %d" %len(leafboth))
...
def batchcorrect(Xcurr, Xcrosscurr):
    """takes in two dataframes with rows as samples and cols as genes"""
