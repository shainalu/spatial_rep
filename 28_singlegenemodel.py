"""
using 'single genes' as features for lasso/linear regression
...
#outfiles
TRAIN_OUT = "singlegenes_STtrain0_070220.csv"
TEST_OUT = "singlegenes_STtest0_070220.csv"
CROSS_OUT = "singlegenes_STtoABA0_070220.csv"
...
def getallbyall(mod_data, mod_propont, cross_data, cross_propont):
    ...
    #for i in range(5,6,1):
        ...
        for j in range(i+1,mod_propont.shape[1]): #upper triangular!
            ...
            #if all curent X is 0, just move on
            if (Xcurr.sum().sum()==0 or Xcrosscurr.sum().sum()==0):
                allbyall_train.iloc[i,j] = -1
                allbyall_test.iloc[i,j] = -1
                allbyall_cross.iloc[i,j] = -1
                continue
...
def main():
    ...
    STspots = STspots.loc[:,genes]
    ABAvox = ABAvox.loc[:,genes]
    ...
    #write outfiles
    allbyall_test.to_csv(TEST_OUT, sep=',', header=True, index=False)
    allbyall_train.to_csv(TRAIN_OUT, sep=',', header=True, index=False)
    allbyall_cross.to_csv(CROSS_OUT, sep=',', header=True, index=False)
main()